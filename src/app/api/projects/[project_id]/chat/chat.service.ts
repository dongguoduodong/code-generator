import { streamText, generateObject, CoreMessage, StreamTextResult, LanguageModelV1 } from "ai";
...
export function createTemplateStreamResponse(
  ...
  if (!template) {
    return new NextResponse(
      `Error: Template with ID '${routerDecision.templateId}' not found.`,
      { status: 500 }
    );
  }
    ...
    async start(controller) {
      ...
      if (routerDecision.customInstructions) {
        try {
          ...
          ).replace("{custom_instructions}", routerDecision.customInstructions);
          ...
          if (lineBuffer.startsWith("- ")) {
            push(`\n${stepCounter++}. ${lineBuffer.substring(2)}`);
          }
        } catch (e) {
      ...
      for (const step of template.planExecutionSteps) {
        ...
        push(`${stepCounter++}. ${parameterizedStep}\n`);
        await new Promise((res) => setTimeout(res, 5));
      }
